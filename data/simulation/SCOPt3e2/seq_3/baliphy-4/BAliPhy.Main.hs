...
sample_smodel  alpha = do {pi <- sample (symmetricDirichletOn (getLetters alpha) 1)
;let {result = ((SModel.lg alpha +> SModel.plus_f' alpha pi) +> unitMixture) +> SModel.gammaRates 1.77721262122212 5}
;let {loggers = ["f:pi" %=% pi]}
;return (result, loggers)
}

sample_imodel  topology = IModel.rs07 0.184394808144225 4.47598050769384 topology
...
;let {substs = parsimony tree (unitCostMatrix aa) (sequenceData, alignment)}
...
;let {scale = scale1}
;let {loggers = ["indelRates" %>% log_indelRates, "|T|" %=% tlength, "scale1" %=% scale1, "scale1*|T|" %=% (scale1 * tlength), "S1" %>% log_smodel, "P1" %>% p1_loggers, "scale" %=% scale, "scale*|T|" %=% (scale * tlength), "|A|" %=% sum alignmentLengths, "#indels" %=% sum [num_indels], "|indels|" %=% sum [total_length_indels], "#substs" %=% sum [substs], "prior_A" %=% sum [prior_A]]}
;
;
...
;sequenceData <- mkUnalignedCharacterData aa <$> loadSequences "data/simulation/SCOPt3e2/seq_3/sequences.fasta"
;
